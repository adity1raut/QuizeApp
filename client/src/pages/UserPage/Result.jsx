...
const Results = () => {
    ...
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-300">Loading your results...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        <div className="text-center max-w-md mx-auto p-6">
          ...
          <h3 className="text-xl font-medium text-red-400 mb-2">Results Not Available</h3>
  ...
  const getPerformanceLevel = (score) => {
    if (score >= 90) return { level: "Excellent", color: "text-green-400", icon: <Trophy className="h-5 w-5" /> };
    if (score >= 70) return { level: "Good", color: "text-blue-400", icon: <Award className="h-5 w-5" /> };
    if (score >= 50) return { level: "Average", color: "text-yellow-400", icon: <Star className="h-5 w-5" /> };
    return { level: "Needs Improvement", color: "text-red-400", icon: <AlertCircle className="h-5 w-5" /> };
  };
        ...
        <div className="bg-gray-800 rounded-xl p-6 mb-8 border border-gray-700">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <h2 className="text-xl text-blue-400 font-semibold">{results.quiz.title}</h2>
            ...
            <div className="mt-4 md:mt-0 flex items-center">
              <div className="text-center bg-gray-700 p-4 rounded-lg">
                <div className="flex items-center justify-center mb-2">
                  {performance.icon}
                  <span className={`ml-2 font-medium ${performance.color}`}>{performance.level}</span>
                </div>
          ...
          <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-white font-semibold text-xl">{results.correctAnswers}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-white font-semibold text-xl">{results.totalQuestions - results.correctAnswers}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-white font-semibold text-xl">{results.score}%</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-4">
            {results.detailedResults.map((result, index) => (
              <div key={index} className="bg-gray-800 rounded-xl overflow-hidden border border-gray-700">
                <div className={`flex items-center px-4 py-3 ${result.isCorrect ? 'bg-green-900 bg-opacity-20' : 'bg-red-900 bg-opacity-20'}`}>
                  {result.isCorrect ? (
                ...
                <div className="p-4">
                  <p className="text-white font-medium mb-4">{result.questionText}</p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className={`p-3 rounded-lg ${result.isCorrect ? 'bg-green-900 bg-opacity-30 text-green-300' : 'bg-red-900 bg-opacity-30 text-red-300'}`}>
