...
const addQuestionsToQuiz = async (req, res) => {
    try {
        ...
        for (let i = 0; i < questions.length; i++) {
            ...
            if (!questionText || !options || !correctAnswer || !Array.isArray(options) || options.length < 2 || !options.includes(correctAnswer)) {
                errors.push(`Question ${i + 1}: Invalid data. Ensure questionText, options (array with >= 2 items), and a valid correctAnswer are provided.`);
                continue;
            }
...
const updateQuestion = async (req, res) => {
    try {
        ...
        if (!questionText || !options || !correctAnswer || !Array.isArray(options) || options.length < 2 || !options.includes(correctAnswer)) {
            return res.status(400).json({ 
                success: false,
                error: 'Invalid data. Ensure questionText, options (array with >= 2 items), and a valid correctAnswer are provided.' 
            });
        }
...
const deleteQuestion = async (req, res) => {
    try {
        ...
        await Quiz.findByIdAndUpdate(question.quiz, { $pull: { questions: questionId } });
