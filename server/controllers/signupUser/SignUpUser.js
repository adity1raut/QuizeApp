...
const signupUser = async (req, res) => {
    try {
        ...
        if (error.message === 'Could not send OTP email.') {
             return res.status(500).json({ message: 'Failed to send OTP email. Please ensure server email credentials are correct.' });
        }
...
const verifyOtpAndCreateUser = async (req, res) => {
    try {
        ...
        if (!tempUser || Date.now() > tempUser.expires) {
            ...
            return res.status(400).json({ message: 'OTP is invalid or has expired. Please sign up again.' });
        }
        ...
        if (user) {
            ...
            res.status(400).json({ message: 'Failed to create user. Invalid data provided.' });
        }
